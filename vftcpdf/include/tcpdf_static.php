...
class VFTCPDF_STATIC {
	...
	public static function getObjFilename($type='tmp') {
		return tempnam(VF_K_PATH_CACHE, '__tcpdf_'.$type.'_'.md5(getmypid().uniqid('', true).rand().microtime(true)).'_');
	}
	...
	* @since 5.9.121 (2011-09-28)
	...
	public static function fileGetContents($file) {
		...
		if (preg_match('%^/{2}%', $url)) {
			if (preg_match('%^([^:]+:)//%i', VF_K_PATH_URL, $match)) {
		...
		if (!isset($urldata['query']) OR (strlen($urldata['query']) <= 0)) {
			if (strpos($url, VF_K_PATH_URL) === 0) {
				// convert URL to full server path
				$tmp = str_replace(VF_K_PATH_URL, VF_K_PATH_MAIN, $url);
