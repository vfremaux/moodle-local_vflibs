...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	public static function getImageFileType($imgfile, $iminfo=array()) {
		...
		if (empty($type)) {
			...
			if (isset($fileinfo['extension']) AND (!VFTCPDF_STATIC::empty_string($fileinfo['extension']))) {
	 ...
	 * This function requires GD library and write access to the directory defined on VF_K_PATH_CACHE constant.
	...
	public static function _parsejpeg($file) {
		...
		while (($pos = strpos($data, "ICC_PROFILE\0", $offset)) !== false) {
			...
			$length = (VFTCPDF_STATIC::_getUSHORT($data, ($pos - 2)) - 16);
	...
	public static function _parsepng($file) {
		...
		$w = VFTCPDF_STATIC::_freadint($f);
		$h = VFTCPDF_STATIC::_freadint($f);
		$bpc = ord(fread($f, 1));
		$ct = ord(fread($f, 1));
		if ($ct == 0) {
			$colspace = 'DeviceGray';
		...
		$icc = false;
		do {
			$n = VFTCPDF_STATIC::_freadint($f);
			$type = fread($f, 4);
			if ($type == 'PLTE') {
				// read palette
				$pal = VFTCPDF_STATIC::rfread($f, $n);
				fread($f, 4);
			} elseif ($type == 'tRNS') {
				// read transparency info
				$t = VFTCPDF_STATIC::rfread($f, $n);
				if ($ct == 0) {
					$trns = array(ord($t{1}));
				} elseif ($ct == 2) {
				...
				$data .= VFTCPDF_STATIC::rfread($f, $n);
				fread($f, 4);
			} elseif ($type == 'iCCP') {
				// skip profile name
				$len = 0;
				...
				$icc = VFTCPDF_STATIC::rfread($f, ($n - $len - 2));
				// decompress profile
