...
require_once(dirname(__FILE__).'/include/VFTCPDF_FILTERS.php');
...
// -----------------------------------------------------------------------------
    ...
    protected function decodeStream($sdic, $stream) {
        ...
        foreach ($filters as $filter) {
            if (in_array($filter, VFTCPDF_FILTERS::getAvailableFilters())) {
                try {
                    $stream = VFTCPDF_FILTERS::decodeFilter($filter, $stream);
                } catch (Exception $e) {
                    $emsg = $e->getMessage();
